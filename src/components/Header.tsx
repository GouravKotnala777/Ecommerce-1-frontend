...
const MyProfileDialog = ({setIsMyProfileDialogOpen}:{setIsMyProfileDialogOpen:Dispatch<SetStateAction<boolean>>;}) => {
    ...
    return(
        <div className="my_profile_dialog_cont" onClick={(e) => e.stopPropagation()}>
            <div className="main_heading">My Profile</div>
            <p style={{fontSize:"0.6rem"}}>{JSON.stringify(updateFormData)}</p>
            {
                isLoading ?
                    <h1>Loading...</h1>
                    :
                    <>
                        <div className="my_details">
                            <div className="heading">Name</div><div className="value">{user?.name} <input type="text" name="name" id="name" placeholder="New Name" value={updateFormData?.name ? updateFormData?.name : ""} onChange={(e) => updateFormChangeHandler(e)} style={{display:editFieldList.includes("name")?"block":"none"}} /> {editFieldList.includes("name") ? <CgClose id="name-CgClose" className="BiEdit" onClick={(e) => func(e)}  /> : <BiEdit id="name-BiEdit" className="BiEdit" onClick={(e) => func(e)} />}</div>
                            <div className="heading">Email</div><div className="value">{user?.email}</div>
                            <div className="heading">Password</div><div className="value">********** <input type="text" name="password" id="password" placeholder="New Password" value={updateFormData?.password ? updateFormData?.password : ""} onChange={(e) => updateFormChangeHandler(e)} style={{display:editFieldList.includes("password")?"block":"none"}} /> {editFieldList.includes("password") ? <CgClose id="password-CgClose" className="BiEdit" onClick={(e) => func(e)}  /> : <BiEdit id="password-BiEdit" className="BiEdit" onClick={(e) => func(e)} />}</div>
                            <div className="heading">Mobile</div><div className="value">{user?.mobile} <input type="text" name="mobile" id="mobile" placeholder="New Mobile" value={updateFormData?.mobile ? updateFormData?.mobile : ""} onChange={(e) => updateFormChangeHandler(e)} style={{display:editFieldList.includes("mobile")?"block":"none"}} /> {editFieldList.includes("mobile") ? <CgClose id="mobile-CgClose" className="BiEdit" onClick={(e) => func(e)}  /> : <BiEdit id="mobile-BiEdit" className="BiEdit" onClick={(e) => func(e)} />}</div>
